...
- `text` sections (except TL;DR — see below)
- `problems` sections
- `process` sections
...
Reserved for deep-dive content that only design managers will read:

- Design decision rationale (why you chose option A over B)
- Iteration logs (what failed and what changed)
- Accessibility or technical edge cases
- Extended reflection / personal learnings beyond a paragraph

---

## The TL;DR Pattern

Every case study should have a **TL;DR text section immediately after the hero image**, with no `visibility` tag. This section is the only prose visible in Visual mode.

It should:
- Be 2–4 short paragraphs max
- Front-load the impact (what changed, what shipped)
- Name the core design challenge
- Reference the key metric

```tsx
{
  type: "text",
  title: "TL;DR",
  // No visibility tag — visible in all modes
  body: (
    <>
      <p>{isPt ? "..." : "..."}</p>
      <p>{isPt ? "..." : "..."}</p>
...
In Visual mode, the reader sees: `metrics → hero image → TL;DR → imageGrids → results`. That is the entire story for a founder.
...
- The metrics section at the top must be punchy and specific
- The TL;DR should open with the outcome, not the problem
...
- [ ] All body text sections (except TL;DR) have `visibility: ["overview", "detailed"]`
- [ ] Problems section has `visibility: ["overview", "detailed"]`
- [ ] Process sections (both `process` type and process imageStacks) have `visibility: ["overview", "detailed"]`
- [ ] Non-hero images (research, sitemaps, artefacts) have `visibility: ["overview", "detailed"]`
- [ ] Visual mode read-through: does it tell a coherent story in under 2 minutes?
- [ ] Overview mode read-through: is the narrative complete and scannable?
